...
class Face_Detector():
	...
	def Detect_Face_Img(self,img,size1,size2):
		...
		for c in contours:
			...
			if (w > size1[0] and h > size1[1]) and (w < size2[0] and h < size2[1]):
				...
				print("\r[CAMS] Pinhole Distance = {:.2f} cm\n[CAMS] Camera distance = {:.2f} cm".format(Distance1,Distance2))
				print("\r[CAMS] Width = {} \t Height = {}".format(w,h))
	...
	def Detect_Face_Vid(self,vid,size1,size2,scale_factor = 3):
		...
		while True:
			...
			print("\r[WEBC] Recording at {} fps".format(fps))
...
if __name__ == "__main__":
	if len(sys.argv) == 1:
		print("[LOGS] Please give me a file :Image/video !!!")
		print("\r[LOGS] Try Again, For more info type --help to see available options")
		sys.exit(0)
	in_arg = Arg_Parser()
